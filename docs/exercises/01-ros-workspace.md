...
!!! exercise "30 minutes, hands-on"
    ...
    **Goal:** a `pixi.toml` and `pixi.lock` that give you a running ROS 2 turtlesim and a custom node of your own, with no ROS installed on the machine, and the same workspace targeting two ROS distributions at once.
